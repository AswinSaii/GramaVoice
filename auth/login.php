...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($phone)) {
        ...
        try {
            ...
            $stmt = $db->prepare("SELECT id, name, phone, verified FROM users WHERE phone = ?");
            ...
            if ($result->num_rows > 0) {
                ...
                if ($user['verified']) {
                    ...
                    $stmt = $db->prepare("UPDATE users SET otp = ? WHERE phone = ?");
                    ...
                    logError("Unverified user login attempt: " . $phone);
                    $error = 'Your account is not verified. Please register first.';
                }
            } else {
                logError("Login attempt with non-existent phone: " . $phone);
                $error = 'Phone number not found. Please register first.';
            }
        } catch (Exception $e) {
            $error_msg = handleException($e, 'Login attempt failed');
...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (phoneInput) {
                // Auto-format phone number with real-time validation
            ...
            document.addEventListener('keydown', function(e) {
                if (e.key === 'Enter' && phoneInput && phoneInput === document.activeElement) {
                    const phone = phoneInput.value;
                    if (validatePhoneNumber(phone)) {
                        loginForm.submit();
                    }
            ...
            // Add screen reader support for validation
